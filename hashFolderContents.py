...
argParser = argparse.ArgumentParser(description='Reads all files in a directory and its subdirectories. It records file paths/sizes/MD5 hashes and writes them to an output file. The output file format is csv with space as the delimiter for the following fields: <hash> <sizeNumber> <sizeHumanReadableText> <path>')
